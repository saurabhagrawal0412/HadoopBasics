	...
	public void map(LongWritable key, Text value, Context context)
		throws IOException, InterruptedException {
			...
			if (rrd.targetUrlsList.size()<=0){
				...
				int numberOfOutgoingURLs= rrd.targetUrlsList.size();
				double rankVal= rrd.rankValue;
				sourceUrl= rrd.sourceUrl;
				StringBuffer lastValue= new StringBuffer();
				for(int targetURL:rrd.targetUrlsList )
				{
					context.write(new LongWritable(targetURL),new Text(String.valueOf(rankVal/(double)numberOfOutgoingURLs)));
					lastValue.append("#");
					lastValue.append(String.valueOf(targetURL));
				}
				context.write(new LongWritable(sourceUrl), new Text(String.valueOf(lastValue)));
